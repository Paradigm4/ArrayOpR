...
test_that("filter numeric fields", {
  assert_df_match(
    RefArray$filter(da >= 10, f_double > 0)$to_df(),
...
test_that("filter string fields: exact, starts_with, ends_with", {
  assert_df_match(
    RefArray$filter(text == 'exact match')$to_df(),
    dplyr::filter(dfArraySource, text == 'exact match')
  )
  
  assert_df_match(
    RefArray$filter(text %starts_with% 'A')$to_df(),
    dplyr::filter(dfArraySource, grepl("^[aA]", text))
  )
  
  assert_df_match(
    RefArray$filter(text %ends_with% 'e')$to_df(),
...
test_that("filter string fields: contains", {
  assert_df_match(
    RefArray$filter(text %contains% 'sit')$to_df(),
    dplyr::filter(dfArraySource, grepl("sit", text))
  )
  assert_df_match(
    RefArray$filter(text %contains% '/')$to_df(),
...
test_that("filter string fields: like", {
  assert_df_match(
    RefArray$filter(text %like% '.*sit.*')$to_df(),
    dplyr::filter(dfArraySource, grepl("sit", text))
  )
  assert_df_match(
    RefArray$filter(text %like% '.*[eo]r')$to_df(),
    dplyr::filter(dfArraySource, grepl("[eo]r$", text))
  )
  assert_df_match(
    RefArray$filter(text %like% '.*(ftp|http).*')$to_df(),
...
test_that("filter NA", {
  assert_df_match(
    RefArray$filter(is_null(text))$to_df(),
    dplyr::filter(dfArraySource, is.na(text))
  )
  assert_df_match(
    RefArray$filter(not_null(text))$to_df(),
    dplyr::filter(dfArraySource, !is.na(text))
  )
  assert_df_match(
    RefArray$filter(is_null(f_double))$to_df(),
    dplyr::filter(dfArraySource, is.na(f_double))
  )
  assert_df_match(
    RefArray$filter(not_null(f_double))$to_df(),
    dplyr::filter(dfArraySource, !is.na(f_double))
  )
})
...
test_that("filter %in% %not_in%", {
  ...
  assert_df_match(
    RefArray$filter(upper %in% !!letterOptions)$to_df(),
    dplyr::filter(dfArraySource, upper %in% letterOptions)
  )
  assert_df_match(
    RefArray$filter(upper %not_in% !!letterOptions)$to_df(),
    dplyr::filter(dfArraySource, ! upper %in% letterOptions)
  )
  
  assert_df_match(
    RefArray$filter(da %in% !!numOptions)$to_df(),
    dplyr::filter(dfArraySource, da %in% numOptions)
  )
  
  assert_df_match(
    RefArray$filter(da %not_in% !!numOptions)$to_df(),
    dplyr::filter(dfArraySource, ! da %in% numOptions)
  )
...
test_that("filter bool fields", {
  ...
  assert_df_match(
    RefArray$filter(f_bool == TRUE)$to_df(),
    dplyr::filter(dfArraySource, f_bool)
  )
  assert_df_match(
    RefArray$filter(f_bool == FALSE)$to_df(),
    dplyr::filter(dfArraySource, !f_bool)
  )
})
...
test_that("filter by scidb functions", {
  assert_df_match(
    RefArray$filter(strlen(text) >= 40)$to_df(),
...
test_that("filter by compound expressions", {
  assert_df_match(
    RefArray$filter(da < 5 || db >= 110)$to_df(),
  ...
  assert_df_match(
    RefArray$filter(da < 5 | (db >= 110 & strlen(text) < 40))$to_df(),
    dplyr::filter(dfArraySource, da < 5 | (db >= 110 & nchar(text) < 40))
  )
  
  assert_df_match(
    RefArray$filter(db >= 110 & strlen(text) < 40)$to_df(),
