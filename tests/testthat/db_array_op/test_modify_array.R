...
test_that("Overwrite array", {
  ...
  conn$array_from_df(df1, Target, force_template_schema = T)$overwrite(Target)$execute()
  expect_df_equal(Target$to_df_all(), df1)
  
  # Field names don't matter
  conn$array_from_df(df2_alt, "<fa2:string,fb:int32> [da;db2]", force_template_schema = T)$overwrite(Target)$execute()
  expect_df_equal(Target$to_df_all(), df2)
  
  conn$array_from_df(df3, Target, force_template_schema = T)$overwrite(Target)$execute()
...
test_that("Update array", {
  ...
  conn$array_from_df(df1, Target, force_template_schema = T)$update(Target)$execute()
  expect_df_equal(Target$to_df_all(), df1)
  
  # Field names don't matter
  conn$array_from_df(df2, "<fa2:string,fb:int32> [da;db2]", force_template_schema = T)$update(Target)$execute()
  expect_df_equal(Target$to_df_all(), df3)
...
test_that("error cases", {
  ...
  dfArray = conn$array_from_df(data.frame(fa = letters[1:2], da=1:2, db=11:12), Target)
