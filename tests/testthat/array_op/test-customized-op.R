...
test_that("Names and types of fields are stored as provided", {
  op = CustomizedOp('raw_afl', c('da', 'db'), c('aa', 'ab'), field_types = list(da='int64', db='int64', aa='string', ab='int32'))
  ...
  expect_identical(op$get_field_names(.DIM), c('da', 'db'))
  expect_identical(op$get_field_names(.ATR), c('aa', 'ab'))
  expect_null(op$get_field_names(.SEL))
...
test_that("Explicitly specify 'validate_fields=T' ", {
  ...
  expect_error(SubsetOp(op, e(non > 3)))
  expect_error(SubsetOp(op, e(da > 0, non == 'val')))
})

test_that("Project selected fields", {
  op = CustomizedOp('raw_afl', c('da', 'db'), c('aa', 'ab'), c('ab', 'aa'))
  assert_afl_equal(op$to_afl(), 'project(raw_afl, ab, aa)')
  op = CustomizedOp('raw_afl', c('da', 'db'), c('aa', 'ab'), c('da', 'aa'))
